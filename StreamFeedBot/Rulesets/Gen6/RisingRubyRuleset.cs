...
{
	public class RisingRubyRuleset : Ruleset
	{
		...
		{
			004, //Leader Viola
			036, //Elite Four Wikstrom
			037, //Elite Four Malva
			038, //Elite Four Siebold
			039, //Leader Grant
			040, //Leader Olympia
			041, //Leader Korrina
			042, //Leader Ramos
			...
			044, //Leader Clemont
			045, //Leader Valerie
			053, //Champion Diantha
			073, //Furisode Girl Iris
			127, //PkMn Trainer Brendan
			128, //PkMn Trainer May
			174, //Aqua Leader
			175, //Aqua Admin Matt
			178, //Magma Leader
			180, //Magma Admin Tabitha
			182, //Magma Admin
			186, //Aqua Admin Shelly
			187, //Magma Admin Courtney
			192, //PkMn Trainer Wally
			194, //Elite Four Sidney
			195, //Elite Four Phoebe
			196, //Elite Four Glacia
			197, //Elite Four Drake
			198, //Champion Steven
			200, //Leader Roxanne
			201, //Leader Brawly
			202, //leader Wattson
			...
			204, //Leader Norman
			205, //Leader Winona
			206, //Leaders Liza & Tate
			207, //Leader Wallace
			219, //PkMn Trainer Steven
			221, //Lorekeeper Zinnia
			268, //Sootopolitan Wallace
			272, //PkMn Trainer Wally
			273, //Elite Four Sidney
			274, //Elite Four Phoebe
			275, //Elite Four Glacia
			276, //Elite Four Drake
			277, //Champion Steven
			278, //PkMn Trainer Brendan
			279, //PkMn Trainer May
		};

		private static readonly int[] SpecialTrainers_ORAS =
		{
			023, //Youngster Joey
			702, //Dragon Tamer Lance
			709, //Ace Trainer Blue
			711, //Ace Trainer Cynthia
		};
		...
		public RisingRubyRuleset(Memory memory, Settings settings)
					...
					case BattleKind.Trainer:
						if (status.EnemyTrainers?.Count == 1)
						{
							if (status.EnemyTrainers[0] != null)
							{
								...
								if (SpecialClasses_ORAS.Contains(trainer.ClassId) || SpecialTrainers_ORAS.Contains(trainer.Id))
								{
						...
						else if (status.EnemyTrainers?.Count == 2)
						{
							if (status.EnemyTrainers[0] != null && status.EnemyTrainers[1] != null)
							{
								...
								if ((SpecialClasses_ORAS.Contains(trainer0.ClassId) || SpecialTrainers_ORAS.Contains(trainer0.Id) || (SpecialClasses_ORAS.Contains(trainer1.ClassId) || SpecialTrainers_ORAS.Contains(trainer1.Id))) && trainer1.ClassId != 0 && trainer1.Id != 0)
								{
									builder.Append($"**VS {trainer0.ClassName} {trainer0.Name} and {trainer1.ClassName} {trainer1.Name}!** ");
									if (SpecialClasses_ORAS.Contains(trainer0.ClassId) || SpecialTrainers_ORAS.Contains(trainer0.Id))
									{
								...
								else if (SpecialClasses_ORAS.Contains(trainer0.ClassId) || SpecialTrainers_ORAS.Contains(trainer0.Id))
								{
				...
				status.GameStats?.Blackouts == oldStatus.GameStats?.Blackouts && !flag)
			{
				...
				else if (oldStatus.EnemyTrainers != null && oldStatus.EnemyTrainers.Count == 2)
				{
					...
					else
					{
						...
						if (SpecialClasses_ORAS.Contains(trainer0.ClassId) || SpecialTrainers_ORAS.Contains(trainer0.Id) ||
							SpecialClasses_ORAS.Contains(trainer1.ClassId) || SpecialTrainers_ORAS.Contains(trainer1.Id))
						{
