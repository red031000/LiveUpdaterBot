...
{
	public class VegaRuleset : Ruleset
	{
		public VegaRuleset(Memory memory, Settings settings)
		...
		private static readonly int[] SpecialClasses_Vega =
		{
			000, //PkMn Trainer
			002, //Team Bh & Wh Leaders
			023, //Hey It's Me Mosmero
			030, //Champion
			047, //Elite Four
			055, //Mos-Ranger
			083, //Champion
		...
		private readonly List<string> _badges = new List<string> { "Elnath", "Gemma", "Hadar", "Arneb", "Phact", "Sarfah", "Prior", "Mirach" };
					...
					case BattleKind.Trainer:
						if (status.EnemyTrainers?.Count == 1)
						{
							if (status.EnemyTrainers?[0] != null)
							{
								...
								if (SpecialClasses_Vega.Contains(trainer.ClassId) || trainer.Id == 099) //joey
								{
						...
						else if (status.EnemyTrainers?.Count == 2)
						{
							if (status.EnemyTrainers[0] != null && status.EnemyTrainers[1] != null)
							{
								...
								if ((SpecialClasses_Vega.Contains(trainer0.ClassId) ||
									    SpecialClasses_Vega.Contains(trainer1.ClassId)) && trainer1.ClassId != 0)
								{
									builder.Append(
								...
								else if (SpecialClasses_Vega.Contains(trainer0.ClassId))
								{
