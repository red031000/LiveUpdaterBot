...
{
	public class SiriusRuleset : Ruleset
	{
		public SiriusRuleset(Memory memory, Settings settings)
		...
		private static readonly int[] SpecialClasses_Sirius =
		{
			000, //PkMn Trainer
			011, //Black Hole Admin
			013, //Black Hole Leader
			031, //Elite Four
			032, //Leader
			038, //Champion
			049, //White Hole Admin
			050, //PkMn Trainer
			053, //White Hole Leader
			058, //Leader
			064, //Pyramid King
		};

		private readonly List<string> _badges = new List<string> { "Kaitos", "Scheat", "Alya", "Spica", "Deimos", "Regulus", "Dios", "Syrma" };
					...
					case BattleKind.Trainer:
						if (status.EnemyTrainers?.Count == 1)
						{
							if (status.EnemyTrainers?[0] != null)
							{
								...
								if (SpecialClasses_Sirius.Contains(trainer.ClassId) || trainer.Id == 322)
								{
						...
						else if (status.EnemyTrainers?.Count == 2)
						{
							if (status.EnemyTrainers[0] != null && status.EnemyTrainers[1] != null)
							{
								...
								if ((SpecialClasses_Sirius.Contains(trainer0.ClassId) ||
									    SpecialClasses_Sirius.Contains(trainer1.ClassId)) && trainer1.ClassId != 0)
								{
									builder.Append(
								...
								else if (SpecialClasses_Sirius.Contains(trainer0.ClassId))
								{
